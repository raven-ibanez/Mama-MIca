...
const MenuItemCard: React.FC<MenuItemCardProps> = ({ 
  ...
  return (
    <>
      <div className={`bg-white rounded-3xl shadow-purple hover:shadow-purple-lg transition-all duration-300 overflow-hidden group animate-scale-in border-2 border-purple-100 ${!item.available ? 'opacity-60' : ''}`}>
        {/* Image Container with Badges */}
        <div className="relative h-56 bg-gradient-to-br from-purple-50 to-purple-100">
          {item.image ? (
            <img
              src={item.image}
              alt={item.name}
          ...
          <div className={`absolute inset-0 flex items-center justify-center ${item.image ? 'hidden' : ''}`}>
            <div className="text-6xl opacity-20 text-purple-400">💎</div>
          </div>
          
          {/* Badges */}
          <div className="absolute top-3 left-3 flex flex-col gap-2">
            {item.isOnDiscount && item.discountPrice && (
              <div className="bg-gradient-purple text-white text-xs font-bold px-4 py-2 rounded-full shadow-purple-lg animate-pulse">
                SALE
              </div>
            )}
            {item.popular && (
              <div className="bg-gradient-to-r from-accent-yellow to-accent-pink text-white text-xs font-bold px-4 py-2 rounded-full shadow-purple-lg">
                ⭐ POPULAR
              </div>
            )}
          </div>
          ...
          {!item.available && (
            <div className="absolute top-3 right-3 bg-neutral-500 text-white text-xs font-bold px-4 py-2 rounded-full shadow-purple-lg">
              UNAVAILABLE
            </div>
          )}
          ...
          {item.isOnDiscount && item.discountPrice && (
            <div className="absolute bottom-3 right-3 bg-white/90 backdrop-blur-sm text-purple-600 text-xs font-bold px-3 py-2 rounded-full shadow-purple-lg">
        ...
        <div className="p-6">
          <div className="flex items-start justify-between mb-4">
            <h4 className="text-xl font-bold text-gradient-purple leading-tight flex-1 pr-2">{item.name}</h4>
            {item.variations && item.variations.length > 0 && (
              <div className="text-xs text-purple-600 bg-purple-100 px-3 py-2 rounded-full whitespace-nowrap font-semibold">
                {item.variations.length} sizes
              </div>
            )}
          </div>
          
          <p className={`text-sm mb-5 leading-relaxed ${!item.available ? 'text-neutral-400' : 'text-neutral-600'}`}>
          ...
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex-shrink-0">
              {!item.available ? (
                <button
                  disabled
                  className="bg-neutral-200 text-neutral-500 px-6 py-3 rounded-2xl cursor-not-allowed font-semibold text-sm"
                >
                  Unavailable
                </button>
              ) : quantity === 0 ? (
                <button
                  ...
                  className="bg-gradient-purple text-white px-8 py-3 rounded-2xl hover:shadow-purple-lg transition-all duration-200 transform hover:scale-105 font-bold text-sm shadow-purple"
                >
                  {item.variations?.length || item.addOns?.length ? 'Customize' : 'Add to Cart'}
                </button>
              ) : (
                <div className="flex items-center space-x-2 bg-gradient-to-r from-purple-100 to-pink-100 rounded-2xl p-2 border-2 border-purple-200">
                  <button
                    onClick={handleDecrement}
                    className="p-2 hover:bg-purple-200 rounded-xl transition-colors duration-200 hover:scale-110"
                  >
                    <Minus className="h-4 w-4 text-purple-700" />
                  </button>
                  <span className="font-bold text-purple-900 min-w-[28px] text-center text-sm">{quantity}</span>
                  <button
          ...
          {item.addOns && item.addOns.length > 0 && (
            <div className="flex items-center space-x-1 text-sm text-purple-600 bg-purple-50 px-3 py-2 rounded-xl font-semibold">
              <span>+</span>
